...
constraints and to ensure that $\alpha$ is a decreasing function. Experimentally, when $\alpha$ was not 
...
\begin{equation} 
	c_{\alpha[n]}(\alpha[n], \alpha[n - 1]) =
  	\begin{cases} 
    	\hfill 1 \hfill & \text{ if $\alpha[n] - \alpha[n - 1] > 0$ } \\
      	\hfill 0 \hfill & \text{ otherwise } \\
  	\end{cases}
\end{equation}

\begin{equation} 
	c_{z_{hip}}(z_{hip}, z_{hip_{max}}) =
  	\begin{cases} 
    	\hfill 1 \hfill & \text{ if $z_{hip} > z_{hip_{max}}$ } \\
      	\hfill 0 \hfill & \text{ otherwise } \\
  	\end{cases}
\end{equation}

\begin{equation} 
	c_{\theta_i}(\theta_i, \theta_{i_{max}}, \theta_{i_{min}}) =
  	\begin{cases} 
      	\hfill 0 \hfill & \text{ if \theta_{i_{max}} > \theta_i > \theta_{i_{min}} } \\
